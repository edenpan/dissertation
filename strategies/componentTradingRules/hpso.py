...
class HierarchinalParticleSwarmOp(pso.ParticleSwarmOp):
	def setRand(self):
		self.randSize = self.iterMax*len(self.particles)*self.dimensions
	...
	def adjParam(self, p):
		...
		while not self.strategy.checkParams(**tempExecParams):	
			for key, value in self.searchParams.iteritems():
				...
				p.v[key] = self.c1*r1[self.randCnt]*(p.best.params[key] - p.params[key]) + self.c2*r2[self.randCnt]*(gbest.params[key] - p.params[key])
				if (p.v[key] == 0):
					if r3[self.randCnt] < 0.5:
						p.v[key] = r4[self.randCnt]  * v
					else:
						p.v[key] = r5[self.randCnt] * v
